...
new_targets <- fread("results_22b/recalculated_targets.csv")
...
write.csv(out_dt, "results_22b/updated_reference_results_22b.csv", row.names = F)
...
dt[, target_deaths_averted := NULL]

setnames(new_targets, "target", "target_deaths_averted", skip_absent = T)
new_targets[v_at_id == 17, v_at_id := 22]

# new_targets[, .(total = sum(target_deaths_averted)), by = year]

dt <- dt[, .(observed_deaths_averted = sum(observed_deaths_averted)),
    by = .(year, disease, location_iso3, region, v_at_id)]
dt <- merge(dt, new_targets, by = c("year", "disease", "v_at_id", "location_iso3"), all.x = T)
# Fill in 2019 observed value as the target for 2019
dt[is.na(target_deaths_averted), target_deaths_averted := observed_deaths_averted] 
...
write.csv(combined_dt, "results_22b/aggregate_updated_reference_results_22b.csv")

## Plots
my_colors1 <- c(RColorBrewer::brewer.pal(name = "Paired", n = 12), c("darkblue", "darkgreen"))
my_colors1[1] <- "hotpink"

pdf("results_22b/updated_reference_results_22b.pdf")
...
write.csv(annual_dt, "results_22b/region_annual_updated_reference_results_22b.csv", row.names = F)
