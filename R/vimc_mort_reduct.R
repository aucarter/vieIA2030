...
vimc_mort_reduct <- function(wpp_input, obs_wpp, vimc) {
    ...
    dt <- left_join(vimc, deaths, by = c("age", "year", "location_iso3")) %>%
        ...
        mutate(mort_reduct = 1 - deaths_obs / (deaths_obs + deaths_averted))

    # Check for missingness
    if (any(is.na(dt$mort_reduct))) {
        missing_locs <- unique(dt[is.na(mort_reduct)]$location_name)
        warning(paste("Missing deaths for", paste(missing_locs, collapse = ", ")))
        dt <- dt[!is.na(mort_reduct)]
    }

    # Check for non-sensical numbers
    if (any(range(dt$mort_reduct) < 0 | range(dt$mort_reduct) > 1)) {
