...
#  - strata_deaths_averted: model estimated (from VIMC) deaths averted
...
run_relative_risk <- function(source = c("vimc", "gbd"), activity = "routine") {
  ...
  # Options for source: "vimc", "gbd", or both any combination of
  # Options for activity: "routine", "campaign", "combined" or both any combination of
...
prep_rr <- function(strata, params) {
  ...
  if (strata$source == "vimc") {
    ...
    # For VIMC diseases, we can take vaccine impact directly
    dt = copy(vimc_impact) %>%
  ...
  if (strata$source == "gbd") {
    ...
    # For non-VIMC diseases, start with deaths attributable to each disease
    dt = copy(gbd_strata_deaths) %>%
      rename(strata_deaths = value) %>%
      mutate(strata_deaths_averted = NA)
...
gbd_rr <- function(dt, p, strata) {
  ...
  # Vaccine efficacy
  strata_efficacy = merge(strata, efficacy)$mean
