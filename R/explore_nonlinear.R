...
explore_nonlinear = function() {
  ...
  #   as.data.table()
  # 
  # y = fread(system.file("extdata", "vimc_coverage.csv", package = "vieIA2030")) %>%
  #   filter(country == "ETH",
  #          disease == "HepB",
  #          vaccine == "HepB_BD") %>%
  #   group_by(year) %>%
  ...
  # 
  # y2 = get_temporal_fvps() %>%
  #   filter(country == "ETH",
  #          d_v_a == "HepB (HepB_BD), routine") %>%
  #   select(year, cov = fvps)
  # 
  # z = x %>%
  #   inner_join(y,  by = "year") %>%
  #   inner_join(y2, by = "year")
  ...
  impact_dt = vimc_impact %>%
    mutate(deaths_averted = pmax(deaths_averted, 0)) %>%
    # Sum impact over age group...
    group_by(country, d_v_at_id, year) %>%
    summarise(impact = sum(deaths_averted)) %>%
    ungroup() %>%
...
plot_model_fits = function(focus, zoom = TRUE) {
  ...
  if (zoom == FALSE)
    max_data$x_max = max(max_data$x_max, x_max)
...
prettify1 = function(g, save = NULL) {
  ...
  lab_dict = c(
    ...
    impact_rel = "Cumulative deaths averted (per FVP per population-person)")
