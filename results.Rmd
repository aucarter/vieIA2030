...
for(r in unique(region_d_dt$region)) {
  ...
  scale_x_continuous(breaks = scales::pretty_breaks())
    print(gg)
    png(paste0("results/", r, "_by_disease.png"))
    print(gg)
    dev.off()
}
```
...
my_colors1 <- RColorBrewer::brewer.pal(name = "Paired", n = 6)

gg <- ggplot(region_y_dt[year %in% 2021:2030], aes(x = year, y = deaths_averted / 1e6 , fill = region)) +
  geom_area(color = "white", alpha = 0.8) +
  scale_fill_manual(values = my_colors1, name = "Region") +
  theme_bw() + xlab("Year") + ylab("Deaths averted (in millions)") +
...
gg <- ggplot(income_group_y_dt[year %in% 2021:2030], aes(x = year, y = deaths_averted / 1e6 , fill = income_group)) +
  geom_area(color = "white", alpha = 0.8) +
  scale_fill_manual(values = my_colors1, name = "Income group") +
  theme_bw() + xlab("Year") + ylab("Deaths averted (in millions)") +
  ggtitle("Deaths averted by income_group") +
  scale_x_continuous(breaks = scales::pretty_breaks())
print(gg)
